...
class GLBench {
    ...
    constructor(gl, settings = {}) {
        ...
        this.updateUI = () => {
            [].forEach.call(this.nodes['recis-gl-gpu-svg'], (node) => node.style.display = this.trackGPU ? 'inline' : 'none');
        };
        ...
        if (!this.withoutUI) {
            ...
            const elm = document.createElement('recis-gl');
            elm.id = 'recis-gl-bench';
            this.dom.appendChild(elm);
            this.dom.insertAdjacentHTML('afterbegin', '<style id="recis-gl-bench-style">' + this.css + '</style>');
            this.dom = elm;
            ...
            this.paramLogger = ((logger, dom, names) => {
                const classes = ['recis-gl-cpu', 'recis-gl-gpu', 'recis-gl-mem', 'recis-gl-fps', 'recis-gl-gpu-svg', 'recis-gl-chart'];
                const nodes = {...classes };
                classes.forEach((c) => nodes[c] = dom.getElementsByClassName(c));
                this.nodes = nodes;
                return (i, cpu, gpu, mem, fps, totalTime, frameId) => {
                    nodes['recis-gl-cpu'][i].style.strokeDasharray = (cpu * 0.27).toFixed(0) + ' 100';
                    nodes['recis-gl-gpu'][i].style.strokeDasharray = (gpu * 0.27).toFixed(0) + ' 100';
                    // eslint-disable-next-line no-nested-ternary
                    nodes['recis-gl-mem'][i].innerHTML = names[i] ? names[i] : (mem ? 'mem: ' + mem.toFixed(0) + 'mb' : '');
                    nodes['recis-gl-fps'][i].innerHTML = 'FPS: ' + fps.toFixed(1);
                    logger(names[i], cpu, gpu, mem, fps, totalTime, frameId);
                };
            })(this.paramLogger, this.dom, this.names);

            this.chartLogger = ((logger, dom) => {
                const nodes = { 'recis-gl-chart': dom.getElementsByClassName('recis-gl-chart') };
                return (i, chart, circularId) => {
                    ...
                    nodes['recis-gl-chart'][i].setAttribute('points', points);
