...
class EventManager {
  ...
  off(event, callback) {
    ...
    else remove(listeners, { callback });
  }
...
function generate() {
  ...
  times(plants, createPlant);

  stems.children().each(function () {
    var tween = TweenMax.to(this, random(2, 4, true), {
      drawSVG: true,
      delay: random(2, true),
...
function createPoints() {
  var x = random(centerX - offsetX, centerX + offsetX);
  var y = 400;
  var dy = 5;
  var offset = 0.007;
  var count = random(30, 55);
  var points = [{ x, y }];
